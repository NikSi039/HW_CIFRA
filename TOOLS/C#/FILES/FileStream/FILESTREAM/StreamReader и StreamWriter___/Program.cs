...
//StreamWriter(string path): через параметр path передается путь к файлу,
//который будет связан с потоком

//StreamWriter(string path, bool append): параметр append указывает,
//надо ли добавлять в конец файла данные или же перезаписывать файл.
...
//параметр encoding указывает на кодировку, которая будет применяться
//при записи
...
//перегруженных версий для записи данных элементарных типов, например,
//Write(char value), Write(int value), Write(double value) и т.д.

//Task WriteAsync(string value): асинхронная версия метода Write.
//Обратите внимание, что асинхронные версии есть не для всех перегрузок
//метода Write.
...
//int Peek(): возвращает следующий доступный символ, если символов
//больше нет, то возвращает -1

//int Read(): считывает и возвращает следующий символ в численном
//представлении. Имеет перегруженную версию:
//Read(char[] array, int index, int count), где array - массив,
//куда считываются символы, index - индекс в массиве array,
...
{
    string text1 = await reader.ReadToEndAsync();
...
string path2 = "/Users/eugene/Documents/app/note1.txt";
