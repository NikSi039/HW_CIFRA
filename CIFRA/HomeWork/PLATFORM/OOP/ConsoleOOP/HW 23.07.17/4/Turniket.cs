...
namespace _4
{
    public class Turniket
    {
        ...
        public void IsValid()
        {
            ...
            {
                case "day":
                    if (ticket.getDatePurchase() == today)
                    {
                        Console.WriteLine($"\n\n\nБилет номер " +
                            $"{ticket.getTicketNumber()} годен");
                        _isValid = true;
                    }
                    else Console.WriteLine($"\n\n\nБилет номер " +
                        $"{ticket.getTicketNumber()} просрочен"); break;
                case "month":
                    if (ticket.getDatePurchase() > today.AddDays(-30))
                    {
                        Console.WriteLine($"\n\n\nБилет номер " +
                            $"{ticket.getTicketNumber()} годен");
                        _isValid = true;
                ...
                case "social":
                    ...
                    _isValid = true; break;
                case "transport":
                    if (ticket.getDatePurchase() > today.AddYears(-1))
                    {
                        Console.WriteLine($"\n\n\nБилет номер " +
                            $"{ticket.getTicketNumber()} годен");
                        _isValid = true;
